package com.github.vinfolhu.config.swagger;
...
public class SwaggerConfig {
	...
	public Docket demoApi() {
		...
		aParameterBuilder.name("Authorization")
				.description(
						"input the token for authentication either in the authorization field or in the token field")
				.modelRef(new ModelRef("string")).parameterType("header").required(false).build();
		ParameterBuilder aParameterBuilder1 = new ParameterBuilder();
		aParameterBuilder1.name("token")
				.description(
						"input the token for authentication either in the authorization field or in the token field")
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(getApiInfo()).useDefaultResponseMessages(false)
				...
				.apis(RequestHandlerSelectors.basePackage(controllerUrl))
//				.apis(RequestHandlerSelectors.basePackage(controllerUrl))
				.build();
	}
